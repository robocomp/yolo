...
Ice.loadSlice("-I ./src/ --all ./src/YoloServer.ice")
import RoboCompYoloServer
...
setattr(RoboCompYoloServer, "ImgType", ImgType)
class Objects(list):
    def __init__(self, iterable=list()):
        super(Objects, self).__init__(iterable)

    def append(self, item):
        assert isinstance(item, RoboCompYoloServer.Box)
...
setattr(RoboCompYoloServer, "Objects", Objects)
...
class Implements:
    def __init__(self, ice_connector, default_handler):
        self.ice_connector = ice_connector
        self.yoloserver = self.create_adapter("YoloServer", yoloserverI.YoloServerI(default_handler))
