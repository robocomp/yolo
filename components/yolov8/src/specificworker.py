...
class SpecificWorker(GenericWorker):
    ...
    def compute(self):
        ...
        if dets is not None:
            ...
            #tracked_boxes, tracked_scores, tracked_cls_inds, tracked_inds = self.track(dets)
            #t4 = time.time()

            #self.objects_write = self.post_process(tracked_boxes, tracked_scores, tracked_cls_inds, tracked_inds)

            #self.objects_write = self.post_process(tracked_boxes, scores, cls_inds, cls_inds, depth, dfocalx, dfocaly, rgb)
...
#                                        class_names=self.yolo_object_predictor.class_names)
    ...
    def track(self, dets):
        ...
        ndets = np.zeros((dets.shape[0], dets.shape[1] - 1))
        ndets[0, :] = dets[0, :-1]
        #print(dets.shape, ndets.shape)
        online_targets = self.tracker.update(ndets, [640, 640], (640, 640))
        tracked_boxes = []
        tracked_scores = []
        tracked_ids = []
    ...
    def post_process(self, final_boxes, final_scores, final_cls_inds, final_inds, depth, focalx, focaly, rgb):   # copy to interface
        ...
        for i in range(len(final_boxes)):
            ...
            cx_i = int((ibox.left + ibox.right)/2)
            cy_i = int((ibox.top + ibox.bot)/2)
            ...
            # ibox.depth = float(np.min(roi[cy_roi-20:cy_roi+20, cx_roi-10:cx_roi+10]))*1000
